...
class F1_Score(BaseMetric):
    ...
    def calculate_metric(self, data):
        #pred_list = data.pred
        pred_list = check_none(data.pred)
        pred_list = [extract_answer(pred) for pred in pred_list]
...
class Fingerprint(BaseMetric):
    ...
    def calculate_metric(self, data):
        ...
        outputs = []
        for pred, golden_answers in zip(pred_list, golden_answers_list):
            gt_mol = Chem.MolFromSmiles(golden_answers[0])
            ot_mol = Chem.MolFromSmiles(pred)
            if ot_mol is None:
                bad_mols += 1
                continue
            outputs.append((gt_mol, ot_mol))
        ...
        for i, (gt_mol, ot_mol) in enumerate(outputs):
            maccs_score = DataStructs.FingerprintSimilarity(MACCSkeys.GenMACCSKeys(gt_mol), MACCSkeys.GenMACCSKeys(ot_mol), metric=DataStructs.TanimotoSimilarity)
            rdk_score = DataStructs.FingerprintSimilarity(Chem.RDKFingerprint(gt_mol), Chem.RDKFingerprint(ot_mol), metric=DataStructs.TanimotoSimilarity)
            morgan_score = DataStructs.TanimotoSimilarity(AllChem.GetMorganFingerprint(gt_mol, self.morgan_r), AllChem.GetMorganFingerprint(ot_mol, self.morgan_r))
            if math.isnan(maccs_score):
                maccs_score = 0
            if math.isnan(rdk_score):
                rdk_score = 0
            if math.isnan(morgan_score):
                morgan_score = 0
            MACCS_sims.append(maccs_score)
            RDK_sims.append(rdk_score)
            morgan_sims.append(morgan_score) 
        
        maccs_sims_score = np.mean(MACCS_sims)
        rdk_sims_score = np.mean(RDK_sims)
        morgan_sims_score = np.mean(morgan_sims)

        return {"validity": validity_score, "maccs_sims": maccs_sims_score, "rdk_sims": rdk_sims_score, "morgan_sims": morgan_sims_score}, [validity_score, maccs_sims_score, rdk_sims_score, morgan_sims_score]
...
class BLEU(BaseMetric):
    ...
    def calculate_metric(self, data):
        ...
        for pred, golden_answers in zip(pred_list, golden_answers_list):
            pred = [pred]
            try:
                bleu_score = self.bleu.compute(predictions=pred, references=golden_answers)['bleu']
        ...
        return {"bleu": np.mean(bleu_scores)}, bleu_scores
    
class Scibench_Score(BaseMetric):
    metric_name = "scibench_score"

    def __init__(self, config):
        super().__init__(config)

    def _extract_answer(self, pred):
        ans = []
        for text in pred:
            match = re.search(r"The answer is therefore[^0-9\-]*([-+]?\d*\.?\d+)", text, re.IGNORECASE)
            if match:
                pattern = r'-?\d+\.?\d*'
                numbers = re.findall(pattern, text)
                ans.append(float(numbers[0]))
            else:
                ans.append(None)
        return ans
    
    def _extract_answer_from_gt(self, golden_answer):
        if isinstance(golden_answer, int) or isinstance(golden_answer, float):
            return golden_answer
        pattern = r'-?\d+\.?\d*'
        numbers = re.findall(pattern, golden_answer)
    ...
    def calculate_metric(self, data):
        ...
        golden_answers_list = self.get_dataset_answer(data)
        pred_list = data.pred
        pred_list = self._extract_answer(pred_list)

        score_list = []
        for pred, golden_answer in zip(pred_list, golden_answers_list):
            if pred is None:
                score_list.append(0)
            else:
                gt = self._extract_answer_from_gt(golden_answer[0])
                score = 1 if math.isclose(pred, gt, abs_tol=tolerance) else 0
                score_list.append(score)

        score = sum(score_list) / len(score_list)
        return {"scibench_score": score}, score_list
...
class MAE(BaseMetric):
    ...
    def _extract_answer(self, pred):
        ...
        for text in pred:
            match = re.search(r"The answer is therefore[^0-9\-]*([-+]?\d*\.?\d+)", text, re.IGNORECASE)
            if match:
                ans.append(float(match.group(1)))
            else:
                ans.append(None)
        return ans
    
    def _remote_whitespace(self, text):
    ...
    def calculate_metric(self, data):
        ...
        for pred, golden_answer in zip(pred_list, golden_answers_list):
            if pred is None:
                ...
                gt = float(self._remote_whitespace(golden_answer[0]))
                mae = abs(pred - gt)
                score_list.append(mae)
        score = sum(score_list) / len(score_list)
        return {"mae": score}, score_list
