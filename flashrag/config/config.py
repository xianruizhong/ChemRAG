...
class Config:
    ...
    def _prepare_dir(self):
        ...
        if self.final_config['do_retrieval']:
            self.final_config["save_dir"] = os.path.join(
                save_dir,
                f"{self.final_config['dataset_name']}_{self.final_config['split']}_{self.final_config['model_name']}_{self.final_config['corpus_name']}_{self.final_config['retrieval_method']}_topk_{self.final_config['retrieval_topk']}_{current_time.strftime('%Y_%m_%d_%H_%M')}_{save_note}",
            )
        else:
            self.final_config["save_dir"] = os.path.join(
                save_dir,
                f"{self.final_config['dataset_name']}_{self.final_config['split']}_{self.final_config['model_name']}_baseline_{current_time.strftime('%Y_%m_%d_%H_%M')}_{save_note}",
            )
