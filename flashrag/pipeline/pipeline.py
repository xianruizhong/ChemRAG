...
class SequentialPipeline(BasicPipeline):
    def __init__(self, config, prompt_template=None, retriever=None, generator=None, inference_only=False, all_corpus=False, retrieval_only=False):
        """
        inference stage:
        ...
        if generator is None and not retrieval_only:
    ...
    def naive_run(self, dataset, do_eval=True, pred_process_fun=None, prompt_template=None):
        # direct generation without RAG
        if prompt_template is not None:
            self.prompt_template = prompt_template
        
        if len(dataset[0].choices) > 0:
            input_prompts = [self.prompt_template.get_string(question=q, choices=c) for q, c in zip(dataset.question, dataset.choices)]
        else:
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=None, prompt_template=None):
        if prompt_template is not None:
            self.prompt_template = prompt_template
        input_query = dataset.question
        
        if not dataset.retrieval_result:
            print("Retrieval results not found, start retrieval")
            retrieval_results, scores = self.retriever.batch_search(input_query, return_score=True)
            assert len(retrieval_results) == len(scores)
            merged_results = []
            for sample_id in range(len(retrieval_results)):
                sub_merge_res = []
                for retrieve_id in range(len(retrieval_results[sample_id])):
                    sub_merge_res.append({
                        "id": retrieval_results[sample_id][retrieve_id]["id"],
                        "contents": retrieval_results[sample_id][retrieve_id]["contents"],
                        "score": scores[sample_id][retrieve_id],
                    })
                merged_results.append(sub_merge_res)
            dataset.update_output("retrieval_result", merged_results)
        else:
            print("Use cached retrieval results")
        ...
        if self.refiner:
            ...
            if not self.use_fid:
                ...
                if len(dataset[0].choices) > 0:
                    input_prompts = [
                        self.prompt_template.get_string(question=q, choices=c, retrieval_result=r[:self.config.retrieval_topk])
                        for q, c, r in zip(dataset.question, dataset.choices, dataset.retrieval_result)
                    ]
                else:
                    input_prompts = [
                        self.prompt_template.get_string(question=q, retrieval_result=r[:self.config.retrieval_topk])
                        for q, r in zip(dataset.question, dataset.retrieval_result)
                    ]
    ...
    def run_retriver(self, dataset, do_eval=True, pred_process_fun=None):
        input_query = dataset.question
        retrieval_results, scores = self.retriever.batch_search(input_query, return_score=True)
        assert len(retrieval_results) == len(scores)
        merged_results = []
    ...
    def evaluate_dataset(self, dataset, pred_process_fun=None):
        dataset, eval_result = self.evaluate(dataset, do_eval=True, pred_process_fun=pred_process_fun)
        return dataset, eval_result
