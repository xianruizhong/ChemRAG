...
def extract_contents(item):
    return item["contents"]
    
def parallel_encode(corpus, num_workers=8):
    with multiprocessing.Pool(processes=num_workers) as pool:
        results = list(tqdm(pool.imap(extract_contents, corpus), total=len(corpus), desc="Loading corpus for encoding"))
    return results
...
class Index_Builder:
    ...
    def _load_embedding(self, embedding_path, corpus_size, hidden_size):
        ...
        #assert all_embeddings.shape[0] == corpus_size
        #assert all_embeddings.shape[1] == hidden_size
    ...
    def encode_all(self):
        encode_data = [item["contents"] for item in tqdm(self.corpus, desc="Loading corpus for encoding")]
        if self.gpu_num > 1:
            print("Use multi gpu!")
...
def main():
    ...
    print(f"embedding_path: {args.embedding_path}")
